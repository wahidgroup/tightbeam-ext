	...
	S: AsyncRead + AsyncWrite + Unpin + Send,
{
	...
	async fn write_frame(&mut self, buffer: &[u8]) -> Result<(), Self::Error> {
		self.inner.send(Message::Binary(buffer.to_vec())).await.map_err(map_ws_error)
	}

	fn is_alive(&self) -> bool {
		// tungstenite exposes no cheap, non-blocking liveness probe; closure is
		// surfaced on the next read as ConnectionClosed.
		true
	}
}
